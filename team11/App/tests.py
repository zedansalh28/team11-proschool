...
class AddTeacherTestCase(TestCase):
    def test_display_add_teacher_form(self):
        response = self.client.get(reverse('add_teacher'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'admin_templates/addTeacher.html')
    ...
    def test_add_new_teacher(self):
        ...
        response = self.client.post(reverse('add_teacher'), data=user_data)
        self.assertEqual(response.status_code, 302)  
        self.assertTrue(User.objects.filter(username='test_teacher').exists()) 
        self.assertTrue(Teacher.objects.filter(user__username='test_teacher').exists())  

    def test_add_teacher_to_group(self):
        user = User.objects.create(username='test_teacher', password='testpassword')
        my_group = Group.objects.create(name='teachers')
        response = self.client.post(reverse('add_teacher'), {'username': user.username})
        self.assertTrue(my_group.user_set.filter(username=user.username).exists())
